...
import model.data_structures.Ayuda.Queue;

public class RedBlackBST2<K extends Comparable<K>, V> {

	private static final boolean RED   = true;
	private static final boolean BLACK = false;
	public Nodo23 principal;
	 private int n;   
	private boolean isRed(Nodo23 x)
	{
		if (x == null)
			return false;
		return x.color == RED;
	...
	public int size(){
	  ...
	  private int size(Nodo23 x) {
	        if (x == null) return 0;
	        return x.size;
	    } 

	public Nodo23 rotateLeft(Nodo23 h)
	{
		Nodo23 x = h.siguiente;
	...
	public Nodo23 rotateRight(Nodo23 h)
	{
		Nodo23 x = h.anterior;
	...
	public void flipColors(Nodo23 h)
	{
		h.color = RED;
	...
	private Nodo23 insert(Nodo23 h, K key, V val) { 
        
		if (h == null){
			
			n++;
			return new Nodo23(key, val, RED, 1);
	...
	public V get(K key){
	 ...
	 public boolean contains(K key){
	   ...
	   private boolean contains(Nodo23 x,K key) {
	  ...
	  private int getHeight(Nodo23 x,K key) {
	  ...
	  public Nodo23 max(){
	   ...
	   private Nodo23 min(Nodo23 x) {
	        if (x.anterior == null) return x; 
	        else                return min(x.anterior); 
	    }
	   private Nodo23 max(Nodo23 x) {
    ...
    private void keys(Nodo23 x, Queue<K> queue) { 
        if (x == null) return; 
        keys(x.anterior, queue); 
        queue.enqueue((K) x.item); 
        keys(x.siguiente, queue); 
    } 
    private void Ks(Nodo23 x, Queue<K> queue) { 
        if (x == null) return; 
        Ks(x.anterior, queue); 
        queue.enqueue((K) x.item); 
    ...
    private boolean isBST(Nodo23<K,V> x, K min, K max) {
    ...
    private boolean isSizeConsistent(Nodo23 x) {
        if (x == null) return true;
    ...
    private boolean isBalanced(Nodo23 x, int black) {
    ...
    private int rank(K key, Nodo23 x) {
        if (x == null) return 0; 
        int cmp = key.compareTo((K) x.item); 
    ...
    private K select(Nodo23 x, int rank) {
