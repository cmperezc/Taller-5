...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public Iterable<Value> valuesInRange(Key init, Key end) {
    	Iterator<Key> it2=KeysInRange(init,end).iterator();
    	Queue<Value> queue3 = new Queue<Value>();
    	while (it2.hasNext() ) {
    ...
    public Iterable<Key> KeysInRange(Key init, Key end) {
    	...
    	Queue <Key> queue2 = new Queue<Key>();
    ...
    private void keys(Node x, Queue<Key> queue) { 
        if (x == null) return; 
        keys(x.left, queue); 
        queue.enqueue((Key) x.val); 
        keys(x.right, queue); 
    } 
    private void Ks(Node x, Queue<Key> queue) { 
        if (x == null) return; 
        Ks(x.left, queue); 
        queue.enqueue((Key) x.val); 
        Ks(x.right, queue); 
